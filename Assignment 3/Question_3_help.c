...
bool firstThread = true;	  // flag used to check if it is the first thread to be running
bool allThreadsReady = false; // flag used to check if all threads are ready
...
int allReady(Thread *threads, int threadCount);
...
int main(int argc, char *argv[])
{
	...
	while (threadsLeft(threads, threadCount) > 0) // put a suitable condition here to run your program
	{
		...
		if (!allThreadsReady)
		{
			int isReady = allReady(threads, threadCount);

			if (isReady) // update global allThreadsRunning
